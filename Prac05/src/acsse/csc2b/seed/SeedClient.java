...
public class SeedClient {
	 ...
	 */
	public SeedClient() {
		SeedClient(DEFAULT_PORT);
	}

	/**
	 * constructor
	 * 
	 * @parameter specific port
	 */
	 ...
	 */
	public void SeedClient(int port) {
		try {
	...
	public static void main(String[] args) {
		...
		SeedClient seeder = new SeedClient();// create seeder on the default port
		...
		while (running) {
			try {
				...
				if (request.equals("LIST")) {
					...
					seeder.getSocket().send(new DatagramPacket(fname.getBytes(), fname.getBytes().length,
							seeder.getIPAddress(), senderPortNumber));

					// send the actual file
					System.out.println("Sending file ID: " + request);
					DatagramPacket packetToSend = sendFile(request, seeder.getIPAddress(), senderPortNumber);
					seeder.getSocket().send(packetToSend);

					// Send an empty packet to signal the end of the file
					seeder.getSocket()
							.send(new DatagramPacket(new byte[0], 0, seeder.getIPAddress(), senderPortNumber));
				} else {
	...
	public static DatagramPacket sendFileList(InetAddress returnAddress, int senderPortNumber) {
		...
		return new DatagramPacket(filesList.getBytes(), filesList.getBytes().length, returnAddress, senderPortNumber);
	}
}
